...
class DBForms {
    ...
    // Метод для отримання N абонентів з найбільшою абонентською платою
    getTopNMobileSpending(N) {
        const subscribers = this.forms
            .flatMap(form => form.ContactDetails
                .map(contact => ({ 
                    fullName: form.fullName, 
                    monthlySubscriptionFee: contact.monthlySubscriptionFee,
                    id: form.id
                }))
                .sort((a, b) => b.monthlySubscriptionFee - a.monthlySubscriptionFee)
                .slice(0, N)
            );
    ...
    getBottomNMobileSpending(N) {
        const subscribers = this.forms
            .flatMap(form => form.ContactDetails
                ...
                .sort((a, b) => a.monthlySubscriptionFee - b.monthlySubscriptionFee)
                .slice(0, N)
            );
